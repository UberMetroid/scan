{
  description = "Minimalist Nix-built container for Scan";
    ...
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        backend = rustPlatform.buildRustPackage {
          ...
          installPhase = ''
            mkdir -p $out/bin
            cp target/release/backend $out/bin/scan-backend
        ...
        dockerImage = pkgs.dockerTools.buildLayeredImage {
          ...
          config = {
            ...
            User = "65534:65534";
            Healthcheck = {
              Test = [ "CMD-SHELL" "wget -qO- http://localhost:4503/health >/dev/null 2>&1 || exit 1" ];
