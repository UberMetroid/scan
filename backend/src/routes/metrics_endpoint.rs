...
mod tests {
    ...
    async fn metrics_endpoint_renders_prometheus_text() {
        ...
        // Match the production leaderboard shape so `read_leaderboard` parses cleanly.
        std::fs::write(
            tmp.path().join("leaderboard.json"),
            br#"{"Alpha":[{"name":"a","score":1,"date":"2026-07-01T00:00:00Z"}]}"#,
        )
        .expect("seed");
        ...
        assert!(
            text.contains("scan_requests_total 3\n"),
            "missing counter; body was: {text}"
        );
        assert!(text.contains("scan_requests_429_total 0\n"));
        assert!(text.contains("scan_leaderboard_entries 1\n"));
        assert!(text.contains("scan_build_info{version=\"9.9.9\"} 1\n"));
    }
}
