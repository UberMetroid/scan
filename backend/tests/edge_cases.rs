...
async fn pwa_manifest_overrides_site_title() {
    ...
    assert_eq!(status, StatusCode::OK);
    assert_eq!(body["name"], "Scan");
    assert_eq!(body["short_name"], "Scan");
    assert_eq!(body["description"], "An orbital hazard sector scanner");
}
...
async fn index_html_replaces_site_title_placeholder() {
    ...
    let text = std::str::from_utf8(&body).expect("utf8");
    assert!(
        text.contains("<title>Scan</title>"),
...
async fn leaderboard_post_rejects_unknown_session_cookie() {
    ...
    let req = Request::builder()
        ...
        .header("cookie", "SCAN_PIN=some-unknown-token")
        .header("origin", "http://localhost:4401")
        .body(Body::from(
            json!({ "name": "x", "score": 1, "category": "Alpha" }).to_string(),
        ))
        .expect("req");
...
async fn leaderboard_concurrent_submissions_do_not_lose_data() {
    ...
    for i in 0..10 {
        ...
        handles.push(tokio::spawn(async move {
            let name = format!("P{}", (b'A' + i as u8) as char);
            let entry = json!({
                "name": name,
                "score": i as u32,
                "category": "Alpha",
