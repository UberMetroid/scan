...
async fn metrics_endpoint_returns_prometheus_text_with_expected_lines() {
    ...
    assert!(text.contains("scan_requests_total "));
    assert!(text.contains("scan_requests_429_total "));
    assert!(text.contains("scan_active_sessions 2\n"));
    assert!(text.contains("scan_leaderboard_entries 1\n"));
    assert!(text.contains("scan_build_info{version=\"test\"} 1\n"));
}
